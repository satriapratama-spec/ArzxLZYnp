...
-- ==========================================
-- Lzy NOTIFY (pequeña, dorada, transparente)
...
do
	...
	local function getHost()
		...
		local gui = host:FindFirstChild("LzyNotifyHost")
		if not gui then
			gui = Instance.new("ScreenGui")
			gui.Name = "LzyNotifyHost"
	...
	function LzyNotify.Show(title, text, duration)
		duration = tonumber(duration) or 2.5
		title = tostring(title or "Lzy Hub")
...
local Z= "LzyHub_FlightSpeed.txt"
local z= "LzyHub_EggSelectConfig.json"
...
local Lzy_SCRIPT_URL = "https://raw.githubusercontent.com/Israel-Vortex/vortex-x-scripts/refs/heads/main/Official-Vortex-Software/Dev-Project/StealAnEgg.lua"
local function LzyQueueReexec()
    pcall(x)
    local src = string.format('loadstring(game:HttpGet("%s"))()', Lzy_SCRIPT_URL)
    pcall(function()
...
-- ============================================================
-- ADDED FEATURES: Egg ESP (HUD), Server Hop, Webhook (Lzy style)
...
-- Webhook — Lzy gold embed style (not the usual colorful bot style)
local function nyxWebhook(title, description, fields)
	...
	local embed = {
		title = "Lzy · " .. tostring(title or "Event"),
		description = tostring(description or ""),
		color = 0xFFD54A, -- near-white accent
		footer = { text = "Lzy Hub · Steal an Egg" },
...
local function aM(...) h.alive = false pcall(nyxEspStop)pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )
        ...
        local y=r:FindFirstChild( "LzyHub_EggESP" )
        if y then
            y:Destroy()
        end
    end
    ...
    pcall(function(...)
        for r,y in ipairs(game.CoreGui :GetChildren())do
            if y.Name :find( "Lzy_" )or y.Name :find( "LzyHubUI" )or y.Name :find( "WindUI" )then
                y:Destroy()
            end
        end
    end
...
local function oM(...)
    ...
    local function j(e,...)
        ...
        -- Lzy Notify (UI propia dorada / transparente)
        pcall(function()
            local title = tostring(e.Title or e.title or "Lzy Hub")
            local content = tostring(e.Content or e.content or e.Text or "")
            local dur = tonumber(e.Duration or e.duration) or 2.5
            if LzyNotify and LzyNotify.Show then
                LzyNotify.Show(title, content, dur)
            end
        end)
    end
    if r then
        pcall(function(...)
            ...
            if e then
                r.Notify =function(self, y, ...)
                    pcall(function()
                        local title = tostring(y and (y.Title or y.title) or "Lzy Hub")
                    ...
                    if not u then
                        pcall(function(...)
                            (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(y and y.Title or "Lzy Hub" );
...
r:SetTheme("LzyGoldSolid")
        ...
        pcall(function()
            LzyNotify.Show("Login LzyHub", "Login LzyHub", 2)
        end)
        task.wait(0.4)
Window=p
        ...
        pcall(function()
            ...
            infoTab:Button({
                ...
                Callback = function()
                    ...
                    pcall(function()
                        r:Notify({ Title = "Lzy Hub", Content = "Discord copiado.", Duration = 2, Icon = "check-circle" })
                    end)
                end})
            infoTab:Toggle({
                ...
                Callback = function(state)
                    pcall(function()
                        local gui = game:GetService("CoreGui"):FindFirstChild("LzyFPSPing")
                        if not state then
        ...
        Fk.btnWebhookTest = MkTab:Button({
            ...
            Callback = function()
                ...
                nyxWebhook("Test", "steal an egg script webhook is working rn", {
                    ...
                    { name = "Hub", value = "Lzy Hub", inline = true }})
    ...
    local k=Instance.new ( "ScreenGui" )k.Name = "LzyHubUI_v41_5" k.ResetOnSpawn = false k.DisplayOrder = 99999 k.ZIndexBehavior =Enum.ZIndexBehavior.Sibling k.AutoLocalize = false
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "Lzy Hub (Fallback)" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
    local D=Instance.new ( "TextLabel" )D.Size =UDim2.new ( 1 , -90 , 0 , 14 )D.Position =UDim2.new ( 0 , 12 , 0 , 26 )D.BackgroundTransparency = 1 D.Text = "Lzy Hub v1.0" D.TextColor3 =Color3.fromRGB ( 155 , 155 , 155 )D.TextSize = 11 D.Font =Enum.Font.Gotham D.TextXAlignment =Enum.TextXAlignment.Left D.AutoLocalize = false D.Parent =U
...
H( "[+] Initializing Lzy Hub x WindUI v1.0 (Lzy Hub Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )
